...
export async function sendContactEnquiry(
 ...
 try {
  ...
  const templateData = {
   ...
   message: data.message ? formatMessageForHTML(data.message) : undefined,
  }

  // Render templates
  const htmlContent = renderTemplate("contact-enquiry", templateData, "html")
  const textContent = renderTemplate("contact-enquiry", templateData, "text")
