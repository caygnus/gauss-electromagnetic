   ...
   <div className="container-industrial">
    <div className="text-center mb-12">
     ...
     <p className="text-steel max-w-2xl mx-auto">
      Comprehensive range of reactor solutions engineered for diverse industrial
      applications
     </p>
    </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
     {reactors.map((reactor, index) => (
      ...
      >
       ...
       <div className="p-5">
        <h3 className="font-semibold text-primary mb-2">{reactor.title}</h3>
        <p className="text-sm text-steel mb-4 leading-relaxed">
         {reactor.description}
        </p>
        <Button asChild variant="industrial" size="sm" className="w-full">
         <Link href={getProductRoute(reactor.slug)}>View Details</Link>
        </Button>
       </div>
      </div>
     ))}
